...
from models.Nets import CNNCifar, MLP, CNNFashion
...
def model_loader(model_name, dataset_name, device, img_size):
    ...
    if model_name == 'cnn' and dataset_name == 'cifar10':
...
def test_model(net_glob, dataset, idx, local_test_bs, device):
    idx_total = dataset.test_users[idx]
    accuracy, loss = test_img(net_glob, dataset, idx_total, local_test_bs, device)
    acc_value = accuracy.item()
    loss_value = loss.item()
    return acc_value, loss_value


def train_model(net_glob, dataset, idx, local_ep, device, lr, momentum, local_bs):
    net_glob_cp = copy.deepcopy(net_glob).to(device)
    return train_cnn_mlp(net_glob_cp, dataset, idx, local_ep, device, lr, momentum, local_bs)
...
def __convert_numpy_value_to_tensor(numpy_data):
...
def decompress_tensor(data):
    tensor_data = __convert_numpy_value_to_tensor(__decompress_data(data))
    return tensor_data
...
def save_model(user_id, epoch, model):
    # https://pytorch.org/tutorials/beginner/saving_loading_models.html#save-load-state-dict-recommended
    filename = "model_user_" + str(user_id) + "_epoch_" + str(epoch) + ".pt"
    torch.save(model.state_dict(), filename)


def exit_process(exit_sleep):
